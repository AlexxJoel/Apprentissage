...
public class Par4 {
 ...
 public static void main(String[] args) {
        int []vector = new int[4];//crea un objeto llamado arreglo vector unidimencinal en la RAM
    ...
    public static void llenar_vector  (int []vector){//crea el metodo llenar_vector con parametro vector
       ...
       for(int i=0; i<vector.length; i++){//indica el inicio de un ciclo al tamaÃƒ ±o del arreglo unidimencional
           ...
           if (numero%2==0) {//es una decicion
               JOptionPane.showMessageDialog(null,"Par" + numero );//envia mensaje a cada pantalla y la guarda en la variable dato
               vector[i]=numero; //si cumple la condicion se guardara
           }
           else{
               JOptionPane.showMessageDialog(null,"inpar" + numero ); //envia mensaje a cada pantalla y la guarda en la variable dato
       }
    }
    }
