...
<?php
	...
	$result = $mysqli->query("SELECT * FROM usuarios WHERE email='$email'")or die($mysql->error());
	...
	if ($result->num_rows > 0) {
		...
		if ($mysqli->query($sql)) {
			...
			$para_usuario = $email;
			$subject = "Verifica tu cuenta (Desarrolla Aplicaciones Web con conexión a base de datos)";
			$message_body = 'Hola'.$nombre.',Gracias por registarte!
