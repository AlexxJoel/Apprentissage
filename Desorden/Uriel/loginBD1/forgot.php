...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($result->num_rows ===0){
		...
		$hash =$user['hash'];
		$nombre =$user['nombre'];
		$_SESSION['message'] = 'Por favor revisa tu correo <strong>'.$email.'</strong>'.'por un link de confirmación para completar el cambio de contraseña!';
		$para_usuario= $email;
		$subject = 'Cambiar password (DAWCBD)';
		$message_body= ' Hola'.$nombre.',<br> Has pedido un cambio de contraseña ! por favor hacer click en el link para cambiar tu contraseña http://localhost/loginBD/reset.php?email='.$email.'&hash ='.$hash;
